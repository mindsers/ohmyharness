...
| `runtime` | `auto` \| `docker` \| `podman` \| `sbx` | which backend to use; `auto` means docker (the measured one). `podman` speaks docker's CLI and adds a rootless user-namespace mapping; `sbx` is Docker Sandboxes. Both `podman` and `sbx` are explicit opt-ins, unmeasured in `auto` until `omh doctor` confirms them — set them explicitly and run `omh doctor` first |
