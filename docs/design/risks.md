...
`git reset --hard` in the sandbox, those were the only copies. `rm` asks for the count now and stands on it.

The refusal is deliberately narrow: a sandbox that never ran, work already on
the branch, or a repository omh cannot read all remove quietly. `rm` is what a
user reaches for when a session has gone wrong, and a refusal that fired
because the sandbox is unreadable would be omh holding the door shut on the way
out.
