...
Defects in what ships today, measured on 2026-08-21 against git 2.55.0. The
first four gate the design; each lands with its red test first. Between them and
the hardening in step 6 of the order below, this work closes or narrows
[risks](risks.md) 2, 2b, 2c, 4b, 4c, 8b, 8c and 9.

| | evidence |
|---|---|
| **`rm` deletes a branch holding unreviewed commits.** `Session::commits` reads a git *failure* as `0`, and `remove` turns `0` into `branch -D`. A base that resolves only as `origin/<name>` — `default_branch` verifies no local ref — makes `rev-list` fail. | `removed session s01; branch omh/s01 dropped (no commits)`, with one commit on it. Contradicts the invariant [commands.md](../commands.md) states outright. |
| **`worktree add -b` is silently overridden** when the base exists only on the remote: git's DWIM wins and checks out trunk itself. Every review path then refuses, and `s ls` reports a branch that was never created. | `git worktree add -b omh/s02 ../wt2 main` → `Preparing worktree (new branch 'main')`. `--no-guess-remote` does not help; resolving the base to a commit first does. |
| **The carried-secret scan over messages is a regex, not a literal.** `--grep` without `-F` is a POSIX BRE. | `KEY=ab+cd/ef12345==` in a commit message is not matched; `-F` matches it. A carried line containing `[` makes `git log` exit 128 and `--keep` fail permanently for that session. |
