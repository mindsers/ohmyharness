...
| a key that can name a credential never lands in a committed file unasked | the committed file is the default now, so the protection is a property of the key — `src/key.rs`, plus the guarantee that no feature name shadows a key, since the feature path does not consult the table at all |
...
| `set`, `unset`, `use` and `unuse` reach every repo layer that already holds the key | writing one while another still declares it is how a command reports success over a value that never changed — shipped twice |
| a settings key and one of omh's features are never the same name | the two write different shapes into one file, and no error message could resolve which was meant |
