    ...
    for page in &pages {
        ...
        for (n, raw) in &joined {
            ...
            for word in rest.split_whitespace() {
                ...
                // whole before it.
                let typeable = |c: char| c.is_ascii_alphanumeric() || "-_./,:=@+~$|".contains(c);
...
fn a_number_the_session_does_not_have_is_refused_with_the_range() {
    ...
    for spec in ["9", "0", "two", "4-2", "1,1"] {
        let err = cmd::harvest::what_to_keep(&shadow, &session, spec, false, false, &|| Ok(true))
            .unwrap_err()
            .to_string();
        assert!(
...
fn resolved_reads_the_manifest_and_this_repos_settings() {
    ...
    cmd::init::install_bundled(&paths.base(), bundled::Shipped::Base, &out::Ctx::plain()).unwrap();
...
fn only_the_recorded_recipes_run_and_the_file_decides_their_order() {
    ...
    let resolved: BTreeMap<String, bool> = ["fixture/zulu", "fixture/alpha", "fixture/asserted"]
        .iter()
...
fn exchange(markers: &[stack::Marker], has_test: bool, typed: &str) -> (usize, Vec<ask::Answer>) {
...
fn an_edit_omh_cannot_open_at_all_is_never_treated_as_absent() {
    ...
    let outcome = cmd::init::install_bundled(&dest, bundled::Shipped::Adapters, &out::Ctx::plain());
...
///
/// **Production lines only.** The tests live in `main_tests.rs` now — they
/// share helpers and reach the command functions through `dispatch`, so
/// they moved as one file rather than following each command into
/// `src/cmd/` — and `testsrc` is what tells the two apart, here and in every
/// other guard that reads its own source.
///
/// Counting tests here would be wrong in any case: it would reward moving a
/// function out and leaving its guard behind, which is the opposite of what
/// should happen next.
#[test]
...
fn the_entries_omh_could_not_read_become_the_line_that_names_them() {
    ...
    let many =
        cmd::session::unreadable_reason(&["denied".to_string(), "denied".to_string()], "runs", dir)
            .expect("two failures are a reason");
    assert!(many.contains("2 entries"), "plural: {many}");
    assert_eq!(
        many.matches("denied").count(),
        1,
        "the same reason twice is said once: {many}"
    );
}
