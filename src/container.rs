...
pub fn plan(
    ...
    if memory_on {
        mounts.push(Mount {
            host: crate::memory::store(paths),
            guest: PathBuf::from(crate::memory::GUEST_NOTES),
            read_only: false,
            file: false,
        });
    }
...
mod tests {
    ...
    fn nothing_beyond_the_worktree_and_credentials_is_writable() {
        ...
        assert!(guests.iter().any(|g| g == crate::memory::GUEST_NOTES));
    ...
    fn the_note_store_is_mounted_from_omhs_own_directory_never_the_checkout() {
        ...
        let notes = p
            .mounts
            .iter()
            .find(|m| m.guest == Path::new(crate::memory::GUEST_NOTES))
    ...
    /// the length of the session — `omh settings mcp import` is how those
    /// servers come
    ...
    fn a_disabled_feature_takes_its_mounts_too() {
        ...
        let guests: Vec<String> = p
            ...
            .collect();
        assert!(
            !guests.iter().any(|g| g == crate::memory::GUEST_NOTES),
